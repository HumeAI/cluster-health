...
echo Switches: "$SSH_ADDR"
ssh "$SSH_USER@$SSH_ADDR" 'sudo ibswitches' | tr -d '"' | awk '{ print($3, $6) }' > "$IB_SWITCH_LIST"
ssh "$SSH_USER@$SSH_ADDR" 'sudo iblinkinfo --line --switches-only' | grep 'Active/' \
